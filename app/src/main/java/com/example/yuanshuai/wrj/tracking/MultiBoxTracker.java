...
package com.example.yuanshuai.wrj.tracking;
...
import com.example.yuanshuai.wrj.Classifier.Classifier.Recognition;
...
public class MultiBoxTracker {
  ...
  public MultiBoxTracker(final Context context) {
    ...
    boxPaint.setStyle(Style.STROKE); //描边风格，可以画出空心框
    boxPaint.setStrokeWidth(12.0f);//空心框的宽度
  ...
  private Matrix getFrameToCanvasMatrix() {
    return frameToCanvasMatrix;//帧到屏幕的渲染矩阵
  }
...
//    }
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      final RectF trackedPos =
          (objectTracker != null)
              ...
              : new RectF(recognition.location);  //获得识别物体位置

      getFrameToCanvasMatrix().mapRect(trackedPos);
      boxPaint.setColor(recognition.color); //获得识别的颜色

      final float cornerSize = Math.min(trackedPos.width(), trackedPos.height()) / 8.0f;
      canvas.drawRoundRect(trackedPos, cornerSize, cornerSize, boxPaint); //cornerSize是圆角

      final String labelString =
          !TextUtils.isEmpty(recognition.title)
              ? String.format("%s %.2f", recognition.title, recognition.detectionConfidence)
              : String.format("%.2f", recognition.detectionConfidence); //要看有没有识别出物体的名字
  ...
  public synchronized void onFrame(
    ...
    //logger.e("hello %s",objectTracker.toString());
  ...
  private void processResults(
    ...
    for (final Recognition result : results) {
      ...
      rgbFrameToScreen.mapRect(detectionScreenRect, detectionFrameRect);//利用rgbFrameToScreen矩阵将detectionFrameRect矩形转换成detectionScreenRect矩形
      ...
      rectsToTrack.add(new Pair<Float, Recognition>(result.getConfidence(), result));//需要进行跟踪的东西
    }
  ...
  private void handleDetection(
    ...
    for (final TrackedRecognition trackedRecognition : trackedObjects) {
      ...
      final float intersectOverUnion = intersectArea / totalArea;//交集占全集的比例
